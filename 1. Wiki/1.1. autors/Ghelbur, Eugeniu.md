---
title: Eugeniu Ghelbur
node_id: "author:ghelbur_eugeniu"
node_type: "author"
category: autors
field:
  - eines per a agents
  - Obsidian
  - sistemes de coneixement
  - generació d’imatges
affiliation:
sources:
  - https://github.com/eugeniughelbur/gpt-image-cookbook
  - https://github.com/eugeniughelbur/agents-md
  - https://theaioperator.io/p/huge-update-on-obsidian-second-brain
  - https://theaioperator.io/p/i-rebuilt-karpathys-llm-wiki-heres
works:
  - "[[1. Wiki/1.3. models/gpt_image_cookbook]]"
  - "[[1. Wiki/1.3. models/konik_method]]"
  - "[[1. Wiki/1.3. models/model_agents_md]]"
  - "[[1. Wiki/1.3. models/obsidian_architect]]"
  - "[[1. Wiki/1.3. models/obsidian_second_brain_karpathy_rebuilt]]"
related_concepts:
  - "[[1. Wiki/1.2. conceptes/PKM]]"
  - "[[1. Wiki/1.2. conceptes/second_brain]]"
  - "[[1. Wiki/1.2. conceptes/skills]]"
status: active
created: 2026-08-07
updated: 2026-08-07
---

# Eugeniu Ghelbur

## Qui és?

Eugeniu Ghelbur és un desenvolupador i autor de projectes sobre eines per a agents, gestió del coneixement amb Obsidian i generació d’imatges amb IA.

## Àmbit de treball

La seva activitat registrada en aquesta wiki combina interfícies de línia d’ordres, fitxers AGENTS.md, habilitats per a agents i arquitectures de segon cervell.

## Obres i contribucions

- **GPT Image Cookbook**, una caixa d’eines per sistematitzar la generació i edició d’imatges.
- **agents-md**, eina per generar i mantenir instruccions de repositori.
- Desenvolupament d’idees com **The Architect** i la reconstrucció del LLM Wiki de Karpathy en un sistema de coneixement més operatiu.

## Relacions

Les seves aportacions es relacionen amb [[1. Wiki/1.2. conceptes/skills]], [[1. Wiki/1.2. conceptes/PKM]], [[1. Wiki/1.2. conceptes/second_brain]] i les fitxes de models indicades a la secció d’obres.

## Fonts

- [GPT Image Cookbook](https://github.com/eugeniughelbur/gpt-image-cookbook)
- [agents-md](https://github.com/eugeniughelbur/agents-md)
- [Huge update on Obsidian Second Brain](https://theaioperator.io/p/huge-update-on-obsidian-second-brain)
- [I rebuilt Karpathy’s LLM Wiki](https://theaioperator.io/p/i-rebuilt-karpathys-llm-wiki-heres)
